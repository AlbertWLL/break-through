...
public class DanqueMQConsumer {
    ...
    public void messageConsumer(Message message, String mqMessage, Channel channel) throws Exception {
        ...
        try {
            ...
            log.info("DanqueMQConsumer-messageConsumer，consumer message success!");
        }catch (Exception e){
     ...
     * @throws Exception
     */
    @RabbitListener(queues = "${danque.rabbitmq.dead.queue.name}")
    @RabbitHandler
    public void deadMessageConsumer(Message message, String mqMessage, Channel channel) throws Exception {
        //订阅队列消息控制接收速率  每次从队列中取一条消息
        channel.basicQos(1);
        log.info("DanqueMQConsumer-messageConsumer，receive message：{} ", mqMessage);
        //业务处理
        try {
            ...
            //Reject表示拒绝消息。requeue：false表示被拒绝的消息是丢弃；true表示重回队列
            //channel.basicReject(message.getMessageProperties().getDeliveryTag(),false);
            log.info("DanqueMQConsumer-deadMessageConsumer，consumer message success!");
        }catch (Exception e){
            log.error("DanqueMQConsumer-messageConsumer-error：{}", e);
            //nack表示拒绝消息。multiple表示拒绝指定了delivery_tag的所有未确认的消息，requeue表示是不是重回队列
            channel.basicNack(message.getMessageProperties().getDeliveryTag(), false,true);
